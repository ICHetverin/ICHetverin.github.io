...
var BTN1 = {
    ...
    get height() {
        return MENU.height/2 - MENU.delta*1.75
    },
    get x() {
...
var BTN2 = {
    ...
    get y() {
        return MENU.y + MENU.height/2 + MENU.delta*0.25
    },
}

var background = new Image();
var imgStal = new Image();
...
function allImages() {
    background.src = "../images/background/moscow.png";
    imgStal.src = "../images/models/stalin.png";
}
...
function drawBtn() {
    ...
    canvasContext.fillText(BTN1.text, BTN1.x + BTN1.width/2, BTN1.y+MENU.delta/1 + BTN1.height/2);

    canvasContext.fillStyle = 'black';
    canvasContext.fillRect(BTN2.x, BTN2.y, BTN2.width, BTN2.height);
    canvasContext.fillStyle = 'white';
    canvasContext.textAlign='center';
    canvasContext.fillText(BTN2.text, BTN2.x + BTN2.width/2, BTN2.y+MENU.delta/2 + BTN2.height/2);
}
...
function drawLeftCharacter() {
    let x = (MainScreen.width - imgStal.width) / 15;
    let y = (MainScreen.height - imgStal.height) / 20;
    canvasContext.drawImage(imgStal, x, y);
}

// function drawRightCharacter() {
//     let x = (MainScreen.width - imgHit.width*1.5)*1.5;
//     let y = (MainScreen.height - imgHit.height) / 20;
//     canvasContext.drawImage(imgStal, x, y)
// }

function draw() {
    let x = (MainScreen.width - background.width)/2;
    let y = (MainScreen.height - background.height)/1.2;
...
window.addEventListener("click", e => {
    ...
    if (
        ...
        window.location.href = 'level4Copy.html'
    }

    if (
        BTN2.x < x && x < BTN2.x + BTN2.width
        && BTN2.y < y && y < BTN2.y + BTN2.height
    ) {
        window.location.href = 'level4Copy2.html'
    }
